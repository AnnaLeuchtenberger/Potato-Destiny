...
function gamePlay(number=currentAction) {
    currentAction = allActions[number].number
    ...
    if (happenedEver("28") && !happenedEver("34")) {
        ...
        let newinventory = inventory.map(toQuestionMarks)
        $inventory.innerHTML = newinventory.join('<br/>');
        $inventoryadd.innerHTML = '';
        $inventoryremove.innerhTML = '';
    } else if (happenedLast("34")) {
        $inventorytitle.innerHTML = '*?(*(*())) ? (*(*(*(*?((';
        let oldInventory = inventory;
        inventory = [];
        $inventory.innerHTML = inventory.join('<br/>');
    } else {$inventory.innerHTML = inventory.join('<br/>')
    }
    if (happenedLast("30") || happenedLast("31")) {
        $thebody.style.backgroundColor = "black";
        sizey -= .2;
        $thebody.style.fontSize = `${sizey}em`;
        $thebody.style.margin = "0px";
        $story.innerHTML = "over under | oover uunder | ovver unnder | vora funda";
        $storyascii.innerHTML = [allStories[number]];
        console.log("happened last 30 or 31")
    }
    if (happenedLast("32")) {
        $thebody.style.backgroundColor = "black";
        sizey += .1;
        $thebody.style.fontSize = `${sizey}em`;
        $story.innerHTML = [ascii3];
        $storyascii.innerHTML = [ascii4];
        console.log("happened last 32");
        $inventory.innerHTML = ``;
        $inventorytitle.innerHTML = ``;
    ...
    if (happenedLast("33")) {
        ...
        $choicesstyle.style.textAlign = "center";
        $inventory.innerHTML = ``;
        $inventorytitle.innerHTML = ``;
        $story.innerHTML = [ascii3];
        $storyascii.innerHTML = [ascii4];
        console.log("happened last 33")

    }
    if (happenedLast("34") || hasItem("waaek")) {
        $thebody.style.backgroundColor = "white";
        $thebody.style.borderColor = "black"
        $choicesstyle.style.textAlign = "left";
        $thebody.style.margin = "34px";
    }
    ...
    if (hasItem("salpee")) {
        ...
        //$storyascii.innerHTML = [sleepStories[number]];
        $inventory.innerHTML = ``;
        $inventorytitle.innerHTML = ``;
    ...
    if (!happenedLast("30") && !happenedLast("31") && !happenedLast("33")) {
        $story.innerHTML =  [allStories[number]]; 
        $storyascii.innerHTML = ``;
    }

    console.log("Boolean value of this action: ", allActions[number].gates(), "<br/>", )
    // DISPLAY CHOICE BUTTONS DEPENDING ON WHICH ACTIONS ARE POSSIBLE
}
